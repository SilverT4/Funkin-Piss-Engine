 ...
 */
class ControlsOld extends FlxActionSet {
	...
	public function copyFrom(controls:ControlsOld, ?device:Device) {
		#if (haxe >= "4.0.0")
	...
	inline public function copyTo(controls:ControlsOld, ?device:Device) {
	...
	public function bindFromSettings(?rebind:Bool) {
		if (rebind == true) {
			unbindKeys(Control.LEFT, [ckl[0]]);
			unbindKeys(Control.DOWN, [ckl[1]]);
			unbindKeys(Control.UP, [ckl[2]]);
			unbindKeys(Control.RIGHT, [ckl[3]]);
		}

		ckl = Options.controls.split("");

		bindKeys(Control.LEFT, [ckl[0], FlxKey.LEFT]);
		bindKeys(Control.DOWN, [ckl[1], FlxKey.DOWN]);
		bindKeys(Control.UP, [ckl[2], FlxKey.UP]);
		bindKeys(Control.RIGHT, [ckl[3], FlxKey.RIGHT]);
	}
